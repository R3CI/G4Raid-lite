...
class printer:
    ...
    def success(text):
        log = f'{co.black}{printer.timestamp()}{co.success}SUCCESS {co.reset}»{co.reset} {co.success}[{text}]{co.reset}'
    ...
    def error(text):                                      
        log = f'{co.black}{printer.timestamp()}{co.error}ERROR {co.reset}»{co.reset} {co.error}[{text}]{co.reset}'
    ...
    def locked(text):                                      
        log = f'{co.black}{printer.timestamp()}{co.locked}LOCKED {co.reset}»{co.reset} {co.locked}[{text}]{co.reset}'
    ...
    def debug(text): 
        if get.debug.enabled():                                     
            log = f'{co.black}{printer.timestamp()}{co.debug}DEBUG {co.reset}»{co.reset} {co.debug}[{text}]{co.reset}'
    ...
    def warning(text):                                      
        log = f'{co.black}{printer.timestamp()}{co.warning}WARNING {co.reset}»{co.reset} {co.warning}[{text}]{co.reset}'
    ...
    def ratelimit(text):                                      
        log = f'{co.black}{printer.timestamp()}{co.ratelimit}RATELIMIT {co.reset}»{co.reset} {co.ratelimit}[{text}]{co.reset}'
    ...
    def cloudflare(text):                                      
        log = f'{co.black}{printer.timestamp()}{co.cloudflare}CLOUDFLARE {co.reset}»{co.reset} {co.cloudflare}[{text}]{co.reset}'
    ...
    def captcha(text):                                      
        log = f'{co.black}{printer.timestamp()}{co.captcha}CAPTCHA {co.reset}»{co.reset} {co.captcha}[{text}]{co.reset}'
